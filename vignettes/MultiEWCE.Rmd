...
cat(paste(length(unique(gene_set$gene_symbol)),
          ...
          paste(gene_set$gene_symbol[seq_len(5)],collapse = ", ")))
```
...
list_names <- unique(gene_counts$disease_id)[seq_len(5)]
```
...
Run the following code the replicate the main analysis in the study described [here](https://doi.org/10.1101/2023.02.13.23285820).
