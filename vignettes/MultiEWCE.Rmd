...
# Introduction
The `MultiEWCE` package is an extension of the [EWCE](https://nathanskene.github.io/EWCE/articles/EWCE.html) package. 
...
```{r get_gene_list}
eye_contact_genes <- HPOExplorer::get_gene_lists(phenotypes = "Poor eye contact", 
                                                 phenotype_to_genes = gene_data)

cat(paste("5 genes associated with poor eye contact:",
          paste(eye_contact_genes$Gene[seq_len(5)],collapse = ", ")))
```
...
Here, "LinkID" refers to the ID of each disease.

```{r}
gene_data <- HPOExplorer::load_phenotype_to_genes("genes_to_phenotype.txt")
#### Filter only to those with >=4 genes ####
gene_counts <- gene_data[,list(genes=length(unique(Gene))),by="LinkID"][genes>=4]
list_names <- unique(gene_counts$LinkID)[seq_len(5)]
```
